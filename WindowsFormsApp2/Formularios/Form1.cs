    ...
    public partial class Form1 : Form, iImportExport
    {
        ...
        private void registrarToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    empresa.AgregarCliente(ag.textBox1.Text, ag.textBox2.Text, Convert.ToInt32(ag.textBox3.Text), Convert.ToDouble(ag.textBox4.Text));
                }
                else
                {
                    ag.Close();
                    ...
                    if (ingresar.textBox1.Text == empresa.Admin.Nombre && Convert.ToInt32(ingresar.textBox2.Text) == empresa.Admin.Dni)
                    {
                        ...
                        while (intentos != 3 && Ingresado == false && ing.ShowDialog() == DialogResult.OK)
                        {
                            ...
                            if (empresa.Admin.Contraseña == ing.textBox1.Text)
                            {
                                ...
                                importarReservasToolStripMenuItem1.Visible= true;
                    ...
                    }else
                    {
                        for (int i = 0; i < empresa.ListaClientes.Count; i++)
                        {
                            if (empresa.ListaClientes[i].Nombre == ingresar.textBox1.Text && empresa.ListaClientes[i].Dni == Convert.ToInt32(ingresar.textBox2.Text))
                            {
                                ...
                                importarReservasToolStripMenuItem1.Visible = false;
        ...
        private void casaToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                bool aux = false;
                while (crearCasa.ShowDialog() == DialogResult.OK && aux == false)
                {
                    ...
                    else MessageBox.Show("Error al cargar la direccion!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                if (btnVerCasas.Enabled == false)
                {
                    cbLista.Items.Clear();
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                empresa.Administrador("Nicolas", 45757987, "Nicolas");
            }
        ...
        private void hotelToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                while (crearhotel.ShowDialog() == DialogResult.OK && aux == false)
                {
                    if (crearhotel.txtNombreHotel.Text != "")
                    {
                        empresa.AgregarHotel(crearhotel.txtNombreHotel.Text, (Hotel.HotelEstrella)crearhotel.cbEstrellas.SelectedIndex, Convert.ToDouble(crearhotel.nudValorBase.Value), crearhotel.Cantidad, crearhotel.Directorio, crearhotel.txtDestino.Text.ToLower(), crearhotel.txtDireccion.Text, Convert.ToInt32(crearhotel.txtNroPropiedad.Text));

                        break;
                    }
                    else MessageBox.Show("Error al cargar el Nombre del Hotel!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                if (BtnVerHoteles.Enabled == false)
                {
                    cbLista.Items.Clear();
        ...
        private void Lista1_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                else hotel = empresa.ListaHoteles[cbLista.SelectedIndex];
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                lr.listBox1.Items.Add("Huespedes");
                for (int i = 0; i < huesped.Length; i++)
                {
                    lr.listBox1.Items.Add(huesped[i]);
                }
                if (res.Casa != null)
                {
                ...
                float posY = 10;
                float posX = 10;
                while (renglon < lr.listBox1.Items.Count)
                {
                    Font fuente = new Font("Giorgia", 10, FontStyle.Bold);
                    string textoActual = lr.listBox1.Items[renglon].ToString();
                ...
                if (res.Casa != null)
                {
                    Image imagen = res.Casa.Images[0];
                    e.Graphics.DrawImage(imagen, 170, 300, 300, 200);
                }
                else
                {
                    Image imagen = res.Hotel.ImagenHotel[0];
                    e.Graphics.DrawImage(imagen, 170, 300, 300, 200);
                }
            }
            catch (Exception)
        ...
        private void exportarClientesToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                 fs = new FileStream(@"Backup\Clientes\ListaClientes.txt", FileMode.OpenOrCreate, FileAccess.Write);
                 sw = new StreamWriter(fs);
                Exportar(fs, sw);
                MessageBox.Show("Se ha guardado con Exito!", "Archivo", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
            }catch (Exception ex) { MessageBox.Show("Error al Exportar Archivo\n" + ex.Message); }
            finally { if (fs != null) { sw.Close(); fs.Close(); } }
        }
